<?php
/**
 * Személyes adatok ellenőrzésének jóváhagyása – az emailben kapott link hívja meg.
 * GET ?token=XXXXX
 */
...
if ((int)$user['data_verified'] === 1) {
    showPage('Már jóváhagyva', 'A felhasználó <b>' . htmlspecialchars($user['username']) . '</b> adatai már korábban ellenőrizve lettek.', true);
    exit;
}

// Jóváhagyás: data_verified = 1, token törlése
$upd = $conn->prepare("UPDATE Users SET data_verified = 1, data_verification_token = NULL WHERE id = ?");
...
try {
    ...
    $mail->Body    = "<h2>Kedves " . htmlspecialchars($user['username']) . "!</h2>"
                   ...
                   . "<p><a href='" . htmlspecialchars(SITE_BASE_URL) . "/frontend/UserProfile/withdrawal.php' "
                   . "style='display:inline-block;padding:12px 24px;background:#f5c518;color:#1a1a2e;text-decoration:none;border-radius:6px;font-weight:bold;'>Kifizetés</a></p>"
                   . "<br><p style='color:#888;font-size:12px;'>Üdvözlettel,<br>BetMatchBonus csapata</p>";

    $mail->send();
} catch (MailException $e) {
    error_log('Adatellenőrzés értesítő email hiba: ' . $e->getMessage());
}

showPage(
    'Adatok jóváhagyva!',
    'A felhasználó <b>' . htmlspecialchars($user['username']) . '</b> (' . htmlspecialchars($user['email']) . ') személyes adatai sikeresen ellenőrizve lettek.<br>A felhasználó értesítést kapott emailben.',
    true
);
