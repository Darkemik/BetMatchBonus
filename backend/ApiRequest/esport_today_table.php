...
WHERE m.sport_id = ?
  AND DATE(m.start_utc) = ?
ORDER BY m.is_live DESC, m.start_utc ASC
";
...
if (!$res || $res->num_rows === 0) {
    echo '<div class="no-matches"><i class="fas ' . $sportIcon . '" style="font-size:40px;color:#aaa;margin-bottom:12px;display:block;"></i>Jelenleg nincs mai eSport mérkőzés.</div>';
    $stmt->close();
    return;
}
?>
<table class="matches-table">
    ...
    <tbody>
        <?php while ($row = $res->fetch_assoc()):
            ...
            $scoreDisplay = !empty($row['score']) ? htmlspecialchars($row['score']) : '-';
