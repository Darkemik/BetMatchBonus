<?php
/**
 * Kifizetési igazolás PDF letöltés
 * GET ?id=TRANSACTION_ID
 * 
 * Csak a saját, jóváhagyott (completed) kifizetésekhez elérhető.
 */
session_start();
...
// Tranzakció lekérése
$stmt = $conn->prepare("
    SELECT t.id, t.transaction_id, t.amount, t.payment_method, t.status, t.account_holder, t.account_number, t.created_at, t.updated_at,
           u.username, u.full_name, u.email
    FROM Transactions t
    INNER JOIN Users u ON u.id = t.user_id
    WHERE t.id = ? AND t.user_id = ? AND t.type = 'withdrawal' AND t.status = 'completed'
    LIMIT 1
");
...
if (!$tx) {
    ...
    die('A kifizetési igazolás nem található vagy még nem lett jóváhagyva.');
}
...
$pdf->SetXY(15, 22);
$pdf->Cell(180, 10, hu('KIFIZETÉSI IGAZOLÁS'), 0, 1, 'L');
...
$pdf->Cell(180, 6, hu('Kifizetett összeg:'), 0, 1, 'C');
...
$rows = [
    ...
    [hu('Jóváhagyás dátuma'), date('Y.m.d H:i', strtotime($tx['updated_at']))],
    [hu('Felhasználó'), $tx['username']],
    [hu('Teljes név'), hu($tx['full_name'] ?? '-')],
    ['Email', $tx['email']],
    [hu('Összeg'), number_format((float)$tx['amount'], 0, ',', ' ') . ' FT'],
    [hu('Fizetési mód'), hu('Banki átutalás')],
    [hu('Számlán szereplő név'), hu($tx['account_holder'] ?? '-')],
    [hu('Bankszámlaszám'), $tx['account_number'] ?? '-'],
    [hu('Státusz'), hu('Jóváhagyva')],
];
...
// --- Kimeneti PDF ---
$filename = 'kifizetes_' . $tx['transaction_id'] . '.pdf';
